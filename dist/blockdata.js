...
function handleSaveNewJson() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const receipt = yield callAddJsonStringAPI(stringToStore); // 여기서 fetch 발생
            console.log('callAddJsonStringAPI 응답 받음:', receipt);
            // ... (이후 로직은 이전과 동일)
            const count = yield callGetJsonCountAPI();
            const newIndex = count > 0 ? count - 1 : 0;
            displayResult(`새로 저장된 JSON (인덱스: ${newIndex}) 및 처리 정보`, { index: newIndex, jsonString: stringToStore }, receipt);
        }
        catch (error) {
...
function updateStatus(message, isError = false, receiptForStatus) {
...
// displayResult 함수 수정
function displayResult(action, data, receiptDetails) {
