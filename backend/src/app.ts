// --- 타입 정의 ---
interface ApiResponseAddJsonReceipt {
    message: string;
...
interface ApiResponseCount {
    count: number;
}

interface RetrievedJsonData {
    index: number;
    jsonString: string;
}
...
function updateStatus(
    ...
    receiptForStatus?: ApiResponseAddJsonReceipt // 상태 영역에 간략히 표시할 영수증 정보
): void {
...
function displayResult(
    ...
    receiptDetails?: ApiResponseAddJsonReceipt // 새로 저장된 항목의 트랜잭션 상세 정보
): void {
...
async function callAddJsonStringAPI(stringToStore: string): Promise<ApiResponseAddJsonReceipt> {
    ...
    try {
        ...
        const resultData: ApiResponseAddJsonReceipt | ApiErrorResponse = await response.json(); // 응답 먼저 파싱
        if (!response.ok) {
...
async function callGetJsonCountAPI(): Promise<number> {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData: ApiErrorResponse = await response.json().catch(() => ({ details: response.statusText }));
            throw new Error(`JSON 개수 조회 실패 (서버): ${errorData.details || response.statusText}`);
        }
        const result: ApiResponseCount = await response.json();
...
async function callGetJsonByIndexAPI(index: number): Promise<RetrievedJsonData> {
    ...
    try {
        ...
        const result: RetrievedJsonData = await response.json();
...
async function handleSaveNewJson(): Promise<void> {
    ...
    try {
        const receipt = await callAddJsonStringAPI(stringToStore); // 상세 정보가 포함된 receipt 받음
        ...
        displayResult(
            ...
            { index: newIndex, jsonString: stringToStore }, // RetrievedJsonData 형태로 전달
...
async function handleGetLatestJson(): Promise<void> { // isAfterSave 매개변수 제거, 항상 일반 조회로 간주
    resultAreaDiv.innerHTML = '';
    getLatestJsonButton.disabled = true;
    getLatestJsonButton.classList.add('opacity-50', 'cursor-not-allowed');
    try {
        const count = await callGetJsonCountAPI(); // count API는 내부에서 status 업데이트
        if (count === 0) {
        ...
        const retrievedData = await callGetJsonByIndexAPI(latestIndex); // 내부에서 status 업데이트
        // 기존에 저장된 데이터를 조회하는 것이므로, 여기서는 receiptDetails를 전달하지 않음
